...
impl Client {
    pub fn connect(addr: &SocketAddr, handle: &Handle) -> Box<Future<Item = Client, Error = io::Error>> {
        let ret = TcpClient::new(CollatzProto).connect(addr, handle).map(|cp| Client { inner: cp });
        Box::new(ret)
    }
}
...
pub struct CollztzCodec {
    decoding_head: bool,
}

impl Decoder for CollztzCodec {
...
impl<T: AsyncRead + AsyncWrite + 'static> ClientProto<T> for CollatzProto {
    ...
    type Error = io::Error;
    type Transport = Framed<T, CollztzCodec>;
    type BindTransport = Result<Self::Transport, io::Error>;
    fn bind_transport(&self, io: T) -> Self::BindTransport {
        let codec = CollztzCodec {
            decoding_head: true,
        };
        Ok(io.framed(codec))
    }
